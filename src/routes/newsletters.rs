...
impl ResponseError for PublishError {
    ...
    // statucs code is invoked by the default error_response
}
...
pub async fn publish_newsletter(
    ...
    // Bubble the error, performing the necessary conversion
    let credentials 
...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    // The header value must be a valid UTF8 string
    let header_value = headers
        .get("Authorization")
        .context("The Authorization hearder was missing")?
        .to_str()
    ...
    // Split into 2 segments using ':' as delimeter
